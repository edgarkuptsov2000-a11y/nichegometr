...
const supabaseKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImplY2RjZGV1ZXhueGNuY3BodHpxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE5NDA1MDIsImV4cCI6MjA4NzUxNjUwMn0.qsF63HwE6GZQJMMtv-4qXWiZAhppeC2rB0bJCxWAL3g";
...
function App() {
  ...
  // ================== ПРОФИЛЬ (users table) ==================
  const getOrCreateProfile = async (uid, emailAddr) => {
    setDebug("DB: читаю профиль из таблицы users…");
    ...
    if (error) {
      // Вот тут будет 401/403 если RLS/права
      throw new Error(
        "Ошибка чтения users. Скорее всего RLS/права. " + error.message
      );
    }

    if (!data) {
      setDebug("DB: профиля нет → создаю…");
      ...
      if (insErr) {
        throw new Error(
          "Ошибка вставки в users. Скорее всего RLS/права. " + insErr.message
        );
      }
      ...
      if (selErr) {
        throw new Error("Создал, но не могу прочитать users: " + selErr.message);
      }
  ...
  // ====== Гарантированная загрузка профиля ======
  const loadProfileFromSession = async (session) => {
    if (!session?.user?.id) {
      setDebug("AUTH: нет session.user после входа (сессия не сохранилась)");
      return;
    }
  ...
  // ====== Подтягиваем сессию при запуске + слушаем изменения ======
  useEffect(() => {
    const init = async () => {
      try {
        setDebug("AUTH: проверяю текущую сессию…");
        ...
        setDebug("INIT ERROR: " + (err?.message || "unknown"));
      }
    };
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(
      async (_event, session) => {
        try {
          ...
          setAuthError(err?.message || "Ошибка после входа");
          setDebug("AUTH LISTENER ERROR: " + (err?.message || "unknown"));
        }
      }
    );
  ...
  const registerWithEmail = async () => {
    ...
    try {
      ...
      alert(
        "Аккаунт создан. Если включено подтверждение почты — подтверди email и зайди."
      );
      setIsLogin(true);
      setDebug("");
    } catch (e) {
      ...
      setDebug("REGISTER ERROR: " + (e?.message || "unknown"));
    } finally {
  ...
  const loginWithEmail = async () => {
    ...
    try {
      ...
      // ВАЖНО: сразу тянем session и грузим профиль (не ждём listener)
      setDebug("AUTH: вход OK → получаю session…");
      ...
      setDebug("LOGIN ERROR: " + (e?.message || "unknown"));
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const handleAuthSubmit = async () => {
    console.log("AUTH CLICK", { isLogin, email });
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("users")
        .update({ nick: updatedUser.nick, avatar: updatedUser.avatar || "" })
  ...
  return (
    <div style={{ padding: "40px 60px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: "60px" }}>
        {/* ====== СТАТИСТИКА (STICKY) ====== */}
        <div style={{ width: "260px", position: "sticky", top: "50px" }}>
          <div
            style={{
              padding: 20,
              background: "#111",
        ...
        {/* ====== ОСНОВНОЙ КОНТЕНТ ====== */}
        <div style={{ flex: 1, maxWidth: "700px", textAlign: "center" }}>
          {/* Аватар + ник */}
          <div
            style={{
              display: "flex",
          ...
          {/* Таймер */}
          <h1 style={{ fontSize: 80, textShadow: "0 0 25px #ff00aa" }}>
            {formatTime(time)}
          </h1>
          ...
          <div className="rating-actions">
            ...
            {ratingLimit === 50 && (
              <button onClick={() => setRatingLimit(100)}>
                Показать ТОП-100
              </button>
            )}
            {ratingLimit === 100 && (
